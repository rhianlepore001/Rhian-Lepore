...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the application's login path at http://localhost:4173/#/login so the gateway with 'Barbearia' card appears.
        await page.goto("http://localhost:4173/#/login", wait_until="commit", timeout=10000)
        
        # -> Click the 'Barbearia' card/button to open the login form (element index 186).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the email and password fields and click the 'ENTRAR' button to log in (input index 246, password index 252, click index 266).
        frame = context.pages[-1]
        # Input text
        ...
        await expect(frame.locator('text=CSV').first).to_be_visible(timeout=3000)
