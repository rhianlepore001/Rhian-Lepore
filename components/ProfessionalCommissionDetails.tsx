                    ...
                    <div className={`p-4 md:p-8 border-b ${colors.divider} ${colors.card} backdrop-blur-md sticky top-0 z-20`}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-4 md:mb-6">
                            <div className={`${colors.surface} ${colors.border} border md:rounded-2xl rounded-xl p-2.5 md:p-4`}>
                                <p className={`text-xs md:text-xs ${colors.textMuted} uppercase ${font.mono} mb-0.5 md:mb-1`}>Serviços</p>
                                <p className={`${colors.text} text-sm md:text-lg font-bold`}>{filteredServices.length}</p>
                            </div>
                            <div className={`${colors.surface} ${colors.border} border md:rounded-2xl rounded-xl p-2.5 md:p-4`}>
                                <p className={`text-xs md:text-xs ${colors.textMuted} uppercase ${font.mono} mb-0.5 md:mb-1`}>Faturamento</p>
                                <p className={`${colors.text} text-sm md:text-lg ${font.mono} font-bold leading-none`}>{currencySymbol} {totalRevenue.toFixed(2)}</p>
                            </div>
                            <div className={`${colors.surface} ${colors.border} border md:rounded-2xl rounded-xl p-2.5 md:p-4`}>
                                <p className={`text-xs md:text-xs ${colors.textMuted} uppercase ${font.mono} mb-0.5 md:mb-1`}>Comissões</p>
                                <p className={`text-sm md:text-lg ${font.mono} font-bold leading-none ${accent.text}`}>{currencySymbol} {totalCommission.toFixed(2)}</p>
                            </div>
                            <div className={`${colors.surface} ${colors.border} border md:rounded-2xl rounded-xl p-2.5 md:p-4`}>
                                <p className={`text-xs md:text-xs ${colors.textMuted} uppercase ${font.mono} mb-0.5 md:mb-1`}>Média por serviço</p>
                        ...
                        <div className={`flex flex-col gap-3 ${colors.surface} p-3 md:p-4 rounded-2xl ${colors.border} border opacity-80`}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className={`${colors.textMuted} text-xs md:text-xs uppercase ${font.mono} block px-1`}>Período de Atendimentos</label>
                                ...
                                <div className="flex flex-col space-y-2">
                                    <label className={`${colors.textMuted} text-xs md:text-xs uppercase ${font.mono} block px-1`}>Atalhos e Filtros</label>
                    ...
                    <div className={`flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar ${colors.surface} opacity-40`}>
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-24 space-y-4">
                                ...
                                <p className={`${colors.textMuted} ${font.mono} text-xs uppercase tracking-widest`}>Sincronizando serviços...</p>
                            </div>
                            ...
                            <div className={`text-center py-20 ${colors.surface} rounded-[32px] border-2 border-dashed ${colors.border}`}>
                                ...
                                <p className={`${colors.textMuted} text-xs mt-1 uppercase ${font.mono}`}>Ajuste os filtros de data acima</p>
                            </div>
                        ) : (
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            <div className="flex items-start gap-3 md:gap-4">
                                                <div className={`hidden md:flex flex-col items-center justify-center min-w-[60px] py-2 ${colors.card} rounded-xl ${colors.border} border`}>
                                                    <span className={`text-xs font-bold ${colors.textMuted} uppercase`}>{new Date(service.appointment_time).toLocaleDateString('pt-BR', { day: '2-digit' })}</span>
                                                    <span className={`text-xs font-bold ${colors.textSecondary} uppercase leading-none mt-0.5`}>{new Date(service.appointment_time).toLocaleDateString('pt-BR', { month: 'short' }).replace('.', '')}</span>
                                                </div>
                                                <div>
                                            ...
                                            <div className={`flex items-center justify-between md:justify-end gap-4 md:gap-8 pt-3 md:pt-0 border-t md:border-t-0 ${colors.divider}`}>
                                                <div className="text-left md:text-right">
                                                    <p className={`text-xs ${colors.textMuted} uppercase ${font.mono} font-bold mb-0.5`}>Preço</p>
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-8">
                            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 w-full md:w-auto">
                                ...
                                <div className="text-center md:text-right w-full md:w-auto">
                                    <p className={`${colors.textMuted} text-xs md:text-xs uppercase ${font.mono} font-bold mb-1 tracking-widest leading-none`}>Total Comissões do Período</p>
