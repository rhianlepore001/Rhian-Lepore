...
async def run_test():
    ...
    try:
        ...
        # -> Wait briefly for SPA to load, then navigate to http://localhost:4173/#/login (first explicit test step).
        await page.goto("http://localhost:4173/#/login")
        
        # -> Click the 'Barbearia' business card (element index 113) to reveal the login form.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the email and password fields (indexes 177 and 183) and click the login button (index 197).
        frame = context.pages[-1]
        # Input text
        ...
        current_url = await frame.evaluate("() => window.location.href")
        assert '/#/onboarding' in current_url
        assert await frame.locator("xpath=//*[contains(., 'Onboarding')]").nth(0).is_visible(), "Expected 'Onboarding' to be visible"
