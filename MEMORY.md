...
- **Tema — fix cores hardcoded Agenda/wizard (18 Jul 2026, branch `feature/theme-hardcoded-fixes`, commit `75b1108`, pushed):** wizard "Novo Atendimento" completo (AppointmentWizard + components/appointment/* + SearchableSelect) e `pages/Agenda.tsx` migrados para tokens (`text-theme-*`, `var(--color-*)`). Corrige cards pretos ilegíveis no modo light (bug reportado pelo Rhian com print). Gates verdes (typecheck/lint/build/337 testes). Prints antes/depois nos 4 temas entregues no chat. **Correção em massa EXECUTADA (18 Jul 2026, aprovada pelo Rhian):** 7 commits por fluxo (financeiro `704d63d`, crm `be3a948`, fila `8408f11`, auth `82c25d6`, modais `54061d0`, settings `26fdf45`, ui `f21cfd1`) + baseline do check-design-debt apertado (`f4f23d3`, -179 linhas de dívida no ratchet). 72 arquivos, `ProfessionalSelector.tsx` deletado (dead code). Gates verdes (337/337). Exceções documentadas mantidas: overlays sobre foto, QRs (fila/2FA/Pix), kiosk TV da fila, verde WhatsApp, thumb de toggle, toasts theme-agnostic, decoração Screw. **Fora do escopo (decisão registrada):** `ClientArea.tsx`, `PublicBooking.tsx`, `ForgotPassword.tsx`, `UpdatePassword.tsx` — sistema visual literal próprio; portar exige decisão de produto/redesign.
