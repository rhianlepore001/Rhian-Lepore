-- Fix ambiguous RPC function call by removing duplicate function signatures
-- Applied: 2025-12-16

-- Dropping BOTH versions to ensure clean slate
DROP FUNCTION IF EXISTS get_finance_stats(uuid, date, date);
...
-- Recreate the single, correct version using TIMESTAMP (compatible with ISO strings)
...
BEGIN
  ...
  -- Transactions - Optimized to avoid JOINs on non-existent FKs
