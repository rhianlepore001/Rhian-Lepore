...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the login page at 'http://localhost:4173/#/login' (per explicit test instruction)
        await page.goto("http://localhost:4173/#/login", wait_until="commit", timeout=10000)
        
        # -> Click the 'Barbearia' card to proceed to the login form (click element index 202).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the login form: type email into the email field (index 249), type password into the password field (index 255), then click the 'ENTRAR' button (index 269).
        frame = context.pages[-1]
        # Input text
        ...
        await expect(frame.locator('text=campaign queued').first).to_be_visible(timeout=3000)
