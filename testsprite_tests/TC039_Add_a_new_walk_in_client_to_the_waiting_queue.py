...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Barbearia' business card to proceed to the login screen (click element index 113).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the login email into the email field (element 175), then type the password (element 181), then click 'ENTRAR' (element 195).
        frame = context.pages[-1]
        # Input text
        ...
        assert await frame.locator("xpath=//*[contains(., 'Aguardando')]").nth(0).is_visible(), "Expected 'Aguardando' to be visible"
