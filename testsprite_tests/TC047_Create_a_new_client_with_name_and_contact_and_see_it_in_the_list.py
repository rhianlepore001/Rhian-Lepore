...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the login route at http://localhost:4173/#/login (explicit test step).
        await page.goto("http://localhost:4173/#/login")
        
        # -> Click the 'Barbearia' card (interactive element index 113) to proceed to the login form or workspace selection.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Enter email and password into the login form, then submit the login button (ENTRAR).
        frame = context.pages[-1]
        # Input text
