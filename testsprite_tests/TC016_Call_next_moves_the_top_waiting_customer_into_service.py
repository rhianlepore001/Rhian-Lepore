...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to http://localhost:4173/#/login and wait for the gateway with 'Barbearia' and 'Beauty & Spa' cards to appear.
        await page.goto("http://localhost:4173/#/login", wait_until="commit", timeout=10000)
        
        # -> Click the 'Barbearia' card/button (index 210) to proceed to the login form.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the email (index 250) and password (index 256) fields, then click the 'ENTRAR' button (index 270) to log in.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Navigate to http://localhost:4173/#/login to reach the gateway/login screen as the next immediate action.
        await page.goto("http://localhost:4173/#/login", wait_until="commit", timeout=10000)
        
        # --> Assertions to verify final state
        frame = context.pages[-1]
        assert '/' in frame.url
        await expect(frame.locator('text=Cliente Chamar').first).to_be_visible(timeout=3000)
