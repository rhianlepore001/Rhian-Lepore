    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                ...
                {/* Card 1: Receita do Mês — sempre exibido, crescimento semanal só com dados */}
                <BrutalCard
                    ...
                    <div className="p-4 md:p-5 flex flex-col justify-between h-full bg-neutral-900/50">
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-1">
                                <span className="text-xs md:text-sm font-mono uppercase text-text-secondary tracking-wider">Receita Realizada</span>
                                <InfoButton text="Valor efetivamente recebido no mês (agendamentos concluídos). Abaixo, o total ainda em agenda (confirmados + concluídos)." />
                            </div>
                        ...
                        <div>
                            <span className={`text-2xl md:text-3xl font-bold font-heading ${accentText}`}>
                                {formatCurrency(metrics.currentMonthRevenue || 0, currencyRegion)}
                            </span>
                            {hasWeeklyData ? (
                            ...
                            {(metrics.monthScheduledValue || 0) > (metrics.currentMonthRevenue || 0) && (
                                <div className="text-xs text-text-secondary/60 font-mono mt-1.5 border-t border-white/5 pt-1.5">
                                    Em agenda: {formatCurrency(metrics.monthScheduledValue || 0, currencyRegion)}
                                </div>
                            )}
                        </div>
                    </div>
