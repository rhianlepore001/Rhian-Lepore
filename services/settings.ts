...
export async function fetchProfileFields(
  ...
  const { data, error } = await supabase
    .from('profiles')
    .select('business_name, phone, address_street, instagram_handle, logo_url, cover_photo_url, business_slug, monthly_goal, daily_goal, public_booking_enabled, booking_lead_time_hours, max_bookings_per_day')
    .eq('id', userId)
    .single();
