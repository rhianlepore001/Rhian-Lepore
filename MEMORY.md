...
- **P0 tenant guard em RPCs autenticadas de financeiro/dashboard (18 Set 2026):** PR #85 (`cursor/p0-tenant-guard-finance-rpcs-b2d0`). Núcleo útil do #79 (fechado), sem identity lock. Migration `20260918220000` aplicada no remoto. Booking público não foi alterado por esta PR.
...
- **Clube — menu do dono + área do cliente — MERGED → produção (5 Set 2026):**
  ...
  - Cliente: plano, validade, incluso e usos em `/#/minha-area/:slug`; cancelar; rota `/#/clube/:slug`.
  - RPCs `get_public_client_membership` e `cancel_public_client_membership` já no banco de produção.
  - Fora deste release: teto de usos no checkout, financeiro da mensalidade, badge CRM.
...
- **Login gateway: tema da sessão vazava no index após logout — MERGED → produção (4 Set 2026):**
  ...
  - E2E: `e2e/login-gateway-theme.spec.ts`. Produção: https://www.agendixstudio.com (deploy Vercel automático).
...
- **Agenda coluna por colaborador (mobile + desktop) — MERGED → produção (2 Set 2026):**
  - Branch `cursor/agenda-colunas-espacamento-d080` · PR #24 → `main` (`0fa10a7`) · CI + Vercel success.
...
- **Varredura Impeccable + Playwright — MERGED → produção (5 Ago 2026):**
  ...
  - Relatório: `docs/impeccable-sweep/`. Residual: accentColor legado em Agenda/Finance/onboarding + HC fora do escopo.
  - Gates: typecheck, lint, build, 400 testes; CI verde no merge.
...
- **Dashboard Cockpit Fase 1 (3 Ago 2026) — MERGED → produção:**
  ...
  - Preview `/#/dashboard-cockpit-demo` + prints live tester.
  - PR #15 merged em `main` (`c61e52e`) → deploy Vercel automático.
