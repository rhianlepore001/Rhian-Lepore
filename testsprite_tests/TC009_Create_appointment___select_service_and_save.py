...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the login route http://localhost:4173/#/login and wait for the gateway cards to appear (look for 'Barbearia' card).
        ...
        await expect(frame.locator('text=Senha').first).to_be_visible(timeout=3000)
        assert '/#/agenda' in frame.url
        await expect(frame.locator('text=Criar agendamento').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Agendamento criado com sucesso').first).to_be_visible(timeout=3000)
