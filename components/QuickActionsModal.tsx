...
export const QuickActionsModal: React.FC<QuickActionsModalProps> = ({ onClose }) => {
    ...
    const modalContent = (
        ...
        bg-[var(--color-modal-bg)] border border-[var(--color-divider)] backdrop-blur-2xl`}
                    role="dialog"
                    aria-modal="true"
                    aria-labelledby="quick-actions-title"
                >
                <div className={`p-4 flex justify-between items-center border-b border-[var(--color-divider)] bg-theme-surface`}>
                    <h3 id="quick-actions-title" className="font-heading font-bold text-theme-text">Ações Rápidas</h3>
                    <button onClick={onClose} className={`p-2 -mr-2 rounded-full transition-colors ${isBeauty ? 'text-[var(--color-text-muted)] hover:text-theme-text hover:bg-[var(--color-card-hover)]' : 'text-accent-gold/60 hover:text-accent-gold hover:bg-accent-gold/10'}`} aria-label="Fechar ações rápidas" title="Fechar">
                        <X className="w-5 h-5" />
                    </button>
                </div>
              ...
              ${isBeauty
                                ? 'border-beauty-neon/20 bg-theme-surface hover:bg-beauty-neon/10 hover:shadow-neon'
                                : 'border-accent-gold/20 bg-theme-surface hover:bg-accent-gold/10 hover:border-accent-gold/40 hover:shadow-gold'}
            `}
                    >
                        <div className={`p-3 rounded-2xl mb-3 ${isBeauty ? 'bg-beauty-neon/15 text-beauty-neon' : 'bg-accent-gold/15 text-accent-gold'}`}>
                            <Calendar className="w-8 h-8" />
                        </div>
                        <span className="font-bold text-sm text-theme-text text-center">Novo Atendimento</span>
                  ...
                  ${isBeauty
                                    ? 'border-green-500/20 bg-theme-surface hover:bg-green-500/10 hover:shadow-[0_0_12px_rgba(52,211,153,0.15)]'
                                    : 'border-green-500/20 bg-theme-surface hover:bg-green-500/10 hover:border-green-500/40 hover:shadow-[0_0_12px_rgba(52,211,153,0.15)]'}
                `}
                        >
                            <div className="p-3 rounded-2xl mb-3 bg-green-500/15 text-green-400">
                                <DollarSign className="w-8 h-8" />
                            </div>
                            <span className="font-bold text-sm text-theme-text text-center">Nova Transação</span>
